...
PTT = r"../WidU_multimodal_fusion/data/raw/ptt_ppg"
...
class LoRALinear(nn.Module):
    def __init__(s, lin, r, a, d):
        ...
        s.lora_A = nn.Linear(i, r, bias=False)
        s.lora_B = nn.Linear(r, o, bias=False)
        s.scaling = a / r
        s.dropout = nn.Dropout(d)
        nn.init.kaiming_uniform_(s.lora_A.weight, a=math.sqrt(5))
        nn.init.zeros_(s.lora_B.weight)

    @property
    def bias(s):
        return s.original.bias

    @property
    def weight(s):
        return s.original.weight

    def forward(s, x):
        return s.original(x) + s.lora_B(s.lora_A(s.dropout(x))) * s.scaling


def inject(m, r=8, a=16, d=0.0):
    for n, mod in list(m.named_modules()):
        if isinstance(mod, nn.Linear) and (
            n.endswith("self_attn.q_proj") or n.endswith("self_attn.v_proj")
        ):
            parts = n.split(".")
            par = m
            for p in parts[:-1]:
...
class Head(nn.Module):
    def __init__(s, o):
        super().__init__()
        s.fc = nn.Linear(768, o)

    def forward(s, x):
        return s.fc(x).squeeze(-1) if s.fc.out_features == 1 else s.fc(x)
...
def load_models(dev):
    ...
    inject(bb)
    ck = torch.load(CKPT_P1, map_location=dev)
...
def main():
    ...
    cs = np.load(os.path.join(CACHET, "signals.npy"))
    cl = np.load(os.path.join(CACHET, "labels.npy"))
    es, mc = run(bb, hb, hm, cs, dev)
    au = roc_auc_score(cl, es)
    # 운영점: 특이도 0.90 고정 시 민감도
    fpr, tpr, th = roc_curve(cl, es)
    spec = 1 - fpr
    i90 = np.searchsorted(spec[::-1], 0.90)
    sens90 = tpr[::-1][i90] if i90 < len(tpr) else float("nan")
    ...
    sig = np.load(os.path.join(CPSC_MC, "signals.npy"))
    lab = np.load(os.path.join(CPSC_MC, "labels.npy"))
    ...
    for cond in ["clean", 12, 6, 0, -6]:
        x = (
            nsr
            if cond == "clean"
            else aug.inject_fixed(
                torch.tensor(nsr, dtype=torch.float32, device=dev), float(cond)
            )
            .cpu()
            .numpy()
        )
    ...
    print(
        "  → 오경보율이 노이즈로 오르면 모션을 ECG 비정상으로 오인 — 결합기에서 차단 필요."
    )
    ...
    for sub in subs:
        for act in ["sit", "walk", "run"]:
            ...
            for st in starts:
                seg = np.zeros((12, 5000), dtype=np.float32)
                seg[LEAD] = ecg[st : st + 5000]
                wins.append(seg)
    ...
    allfar = {}
    for act in ["sit", "walk", "run"]:
        arr = np.array(res_act[act])
        far = (arr >= 0.5).mean()
        allfar[act] = far
        print(f"  {act:>6} {arr.mean():>12.3f} {far:>12.3f}")
    print("  → 운동(run)에서 emergency가 sit 대비 크게 오르면 동성빈맥 오발화 위험.")
    print("     IMU 활동량 룰(고활동→cardiac 억제)이 결합기에서 이를 차단해야 함.")
    print("\n" + "=" * 66 + "\n[척추 완료]\n" + "=" * 66)
