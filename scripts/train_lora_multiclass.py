...
def train(args):
    ...
    # 클래스 가중치 (역빈도)
    counts = np.array([(train_ds.labels == c).sum() for c in range(N_CLASSES)],
                      dtype=np.float64)
    ...
    print(f"{'Epoch':>5} {'TrainLoss':>10} {'ValF1m':>7} "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"{epoch:5d} {avg_loss:10.4f} {metrics['macro_f1']:7.4f} "
    ...
    np.savez(os.path.join(args.out_dir, "test_results.npz"),
             labels=res["labels"], preds=res["preds"], probs=res["probs"],
