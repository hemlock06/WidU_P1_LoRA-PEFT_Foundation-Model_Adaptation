...
class MultiSNRNoise:
    ...
    def __init__(
        ...
        assert noise_mode in ("single", "mixed", "mixed_temporal"), (
    ...
    def inject(
        self, x: torch.Tensor, p_noise: float = 0.75, noise_mode: str = None
    ) -> torch.Tensor:
        """
        ...
        for b in range(B):
            ...
            for c in range(C):
                ...
                if mode == "mixed":
                    ...
                    ntype = NOISE_TYPES[
                        self.rng.choice(len(NOISE_TYPES), p=self.noise_weights)
                    ]
    ...
    def inject_fixed(
        self, x: torch.Tensor, snr_db: float, noise_mode: str = None
    ) -> torch.Tensor:
        """
    ...
    def _mixed_segment(self, length: int, temporal: bool = False) -> torch.Tensor:
        ...
        for i, t in enumerate(NOISE_TYPES):  # (bw, em, ma) 순서 = noise_weights 순서
            ...
            if temporal:
                seg = seg * self._temporal_envelope(
                    t, length, self.rng
                )  # 시간 게이팅(정규화 후)
    ...
    @staticmethod
    def _add_at_snr(
        sig: torch.Tensor, noise: torch.Tensor, snr_db: float
    ) -> torch.Tensor:
...
if __name__ == "__main__":
    ...
    for mode in ("single", "mixed", "mixed_temporal"):
        ...
        for target in targets:
            ...
            ok = (
                err < TOL
                and not torch.isnan(noisy).any()
                and not torch.isinf(noisy).any()
            )
            ok_all &= ok
        ...
        ok_all &= not nan_inf
        print(
            f"[{mode}] inject(p_noise=0.75): 배치 4개 중 clean 유지 {n_clean}개 | "
            f"NaN/Inf={nan_inf}"
        )
    ...
    for t in ("bw", "ma", "em"):
        es = [
            augt._temporal_envelope(t, 5000, augt.rng).cpu().numpy() for _ in range(30)
        ]
    ...
    env_ok = (
        on_fracs["bw"] > 0.95
        and on_fracs["em"] < 0.30
        and on_fracs["ma"] > on_fracs["em"]
    )
    ok_all &= env_ok
