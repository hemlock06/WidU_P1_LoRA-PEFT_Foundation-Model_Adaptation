...
**독거 고령자의 부양자가 원격 돌봄 중 심혈관 응급(급성 허혈·AF)을 놓치지 않으려면, 일상에서 끊김 없이 측정되는 웨어러블 ECG가 필요하다.** 문제는, 정적 임상 12-lead로 사전학습된 모델이 웨어러블의 **모션 노이즈·lead 부족** 환경에서 무력해진다는 점이다.

이 모듈은 그 격차를 메운다 — ECG Foundation Model(ECG-FM)을 **LoRA(0.33% 파라미터)** 로 적응하고 **multi-SNR 모션 증강 + RLM 가변 lead 마스킹** 으로 웨어러블 악조건에 강건하게 만들어, 단일 백본에서 **이진 응급 점수 + 5-class 심장 분류** 를 동시 출력해 P2 융합 모듈로 전달한다.

3-프로젝트 로드맵의 첫 단계: **P1 ECG 분석** → P2 멀티모달 융합(자이로·SpO2) → P3 XAI 설명.
