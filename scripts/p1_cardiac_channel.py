...
class P1CardiacChannel:
    ...
    def __init__(self, device: str = None, ckpt_fm=CKPT_FM, ckpt_p1=CKPT_P1):
        ...
        self.bb = bb.eval()
        self.hb = _Head(1).to(self.device)
        self.hb.load_state_dict(ck["head_bin_state"])
        self.hb.eval()
        self.hm = _Head(5).to(self.device)
        self.hm.load_state_dict(ck["head_mc_state"])
        self.hm.eval()
...
if __name__ == "__main__":
    ...
    for name, m in [("AF(label=1)", cl[:300] == 1), ("정상(label=0)", cl[:300] == 0)]:
        print(
            f"  [{name}] emergency={out['emergency_score'][m].mean():.3f} "
