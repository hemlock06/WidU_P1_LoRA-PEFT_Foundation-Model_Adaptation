...
def get_scores(backbone, head, signals_np, device, lead_mask=None):
    ...
    for start in range(0, N, BATCH):
        x = torch.tensor(signals_np[start : start + BATCH], dtype=torch.float32).to(
            device
        )
...
def test_c():
    ...
    for db_name, sigs, lbl in [
        ...
        (
            "CPSC  emerg   ",
            cpsc_sigs[
                np.load(os.path.join(PROCESSED, "cpsc2018/test/labels.npy")) == 1
            ],
            None,
        ),
        (
            "CPSC  normal  ",
            cpsc_sigs[
                np.load(os.path.join(PROCESSED, "cpsc2018/test/labels.npy")) == 0
            ],
            None,
        ),
    ]:
        ...
        raw_nz = raw[raw != 0]
        print(
            f"    부호 분포: 양={(raw_nz > 0).mean() * 100:.1f}%  음={(raw_nz < 0).mean() * 100:.1f}%"
        )

    # CPSC에서 slot 1,7 이외 lead의 비율 확인 (0-fill 없음 — 항상 12-lead)
    print("\n  ※ CPSC는 12-lead 모두 유효. LTST는 slot 1,7만 유효, 나머지 0.")
    zero_ratio = (
        ltst_sigs[:, [i for i in range(12) if i not in LTST_ACTIVE_SLOTS], :] == 0
    ).mean()
...
def main():
    ...
    parser.add_argument(
        "--tests", nargs="+", default=["A", "B", "C"], choices=["A", "B", "C"]
    )
