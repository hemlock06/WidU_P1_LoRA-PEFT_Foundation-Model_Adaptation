...
목적: 실세계 적용 전, 공개데이터로 단일리드 적용 3축을 실측한다.
  ...
  척추2 NSTDB    : 모션 노이즈를 ECG 비정상으로 오인 안 하는가 + 게이트가 잡는가
...
@torch.no_grad()
def run(bb,hb,hm,hg,x,dev,bs=32):
    es,rel,mc=[],[],[]
    for i in range(0,len(x),bs):
        ...
        rel.append(torch.sigmoid(hg(emb)).cpu().numpy())
        mc.append(torch.softmax(hm(emb),-1).cpu().numpy())
    return np.concatenate(es),np.concatenate(rel),np.concatenate(mc)
...
def main():
    ...
    print(f"  reliability 평균={rel.mean():.3f} (gate alert>{T_ALERT} 비율={100*(rel>=T_ALERT).mean():.1f}%)")
    ...
    print(f"  NSR {len(nsr)}개, emergency 임계 0.5 기준 오경보율 / reliability")
    print(f"  {'조건':>8} {'오경보율':>8} {'rel평균':>8} {'alert비율':>9}")
    for cond in ['clean',12,6,0,-6]:
        ...
        es,rel,mc=run(bb,hb,hm,hg,x,dev)
        far=(es>=0.5).mean(); alert=(rel>=T_ALERT).mean()
        print(f"  {str(cond):>8} {far:>8.3f} {rel.mean():>8.3f} {100*alert:>8.1f}%")
    print("  → 오경보율이 노이즈로 오르면, reliability/alert가 함께 올라 게이트가 ECG 할인 가능해야 함")
    ...
    for sub in subs:
        for act in ['sit','walk','run']:
            ...
            for st in starts:
                ...
                wins.append(seg)
            es,rel,mc=run(bb,hb,hm,hg,np.stack(wins),dev)
            res_act[act].append(es.mean()); rel_act[act].append(rel.mean())
    print(f"  {'활동':>6} {'emergency평균':>12} {'오발화율@0.5':>12} {'rel평균':>8}")
    allfar={}
    for act in ['sit','walk','run']:
        arr=np.array(res_act[act]); far=(arr>=0.5).mean()
        allfar[act]=far
        print(f"  {act:>6} {arr.mean():>12.3f} {far:>12.3f} {np.mean(rel_act[act]):>8.3f}")
