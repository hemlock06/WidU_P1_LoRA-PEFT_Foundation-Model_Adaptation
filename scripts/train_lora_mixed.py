...
def main():
    ...
    parser.add_argument(
        "--ckpt_start",
        default=f"{OUTPUTS}/lora_multisnr/lora_multisnr_best.pt",
    ...
    if args.ckpt_start and os.path.exists(args.ckpt_start):
        ...
        print(
            f"             val AUROC={ckpt_warm.get('val_auroc', 'n/a'):.4f} (원본 CPSC 학습)"
        )
    else:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            f"{epoch:3d} {avg_loss:8.4f} {val_auroc:9.4f} {val_f1:7.4f}{marker}",
            flush=True,
        )
    ...
    for db_name, db_dir in EXT_DIRS.items():
        ...
        try:
            ...
            print(
                f"    {db_name:12s} AUROC={a:.4f}  F1={f:.4f}  Sens@95Sp={s_str}",
                flush=True,
            )
