...
def main():
    ...
    sampler  = WeightedRandomSampler(weights, num_samples=int(n_cpsc * (1 + args.ptbxl_ratio)),
                                     replacement=True)
    ...
    if os.path.isdir(STAFF_DIR):
        ext_loaders["STAFF-III"] = DataLoader(ECGDataset(STAFF_DIR),
                                               batch_size=64, shuffle=False, num_workers=0)
        print(f"         STAFF-III={len(ECGDataset(STAFF_DIR))}")
    if os.path.isdir(LTST_DIR):
        ext_loaders["LTST"] = DataLoader(ECGDataset(LTST_DIR),
                                          batch_size=64, shuffle=False, num_workers=0)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if "STAFF-III" in ext_loaders:
            staff_auroc, _, _ = evaluate(backbone, head, ext_loaders["STAFF-III"], device)

        marker = " ←" if val_auroc > best_auroc else ""
        staff_str = f"{staff_auroc:.4f}" if not np.isnan(staff_auroc) else "  n/a "
        print(f"{epoch:5d} {avg_loss:8.4f} {val_auroc:9.4f} {val_f1:7.4f} {staff_str}{marker}")
