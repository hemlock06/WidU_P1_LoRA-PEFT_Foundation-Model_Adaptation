...
| [records/03_eval_results.md](records/03_eval_results.md) | 평가 결과 — 테스트 지표, SNR 곡선, 외부검증 4종, 5b/5d/5e 비교, **단계 5f P1 최종 수치**, 게이트 임계값 |
| [records/04_run_history.md](records/04_run_history.md) | 실행 기록 — 환경 설정, 다운로드·전처리 이력 (LTST OOM 수정 포함) |
| [records/05_open_issues.md](records/05_open_issues.md) | 이슈 트래커 — LTST/STAFF-III 범위 경계 분석, 7b 임계값, P1 확정, Stage 10, 이소성 표현 천장 |
...
| 7 | 신호품질 게이트 | PhysioNet 2011, AUROC=0.8406 |
| 7b | 게이트 3단 임계값 | t_mask=0.2155 (p75), t_alert=0.4753 (p90, spec≈0.90) |
...
| **11** | **웨어러블 단일리드 적용 연구** | CACHET 단일리드 AF 0.99(낙관적), **reliability 0.68→0.94 재학습**, 온디바이스 추론 CPU 16x (records/03) |
...
|---|---|
| Test AUROC | 0.8406 |
| t_mask | 0.2155 (val 양호 p75) |
| t_alert | 0.4753 (val 양호 p90, spec≈0.90) |
...
| 전용 reliability 헤드 | 깨끗-vs-모션 **0.94** (기존 게이트 0.68), 깨끗 rel 0.06 |
| 신뢰도 가중 신호 | `effective_cardiac = emergency × (1 − 0.6·reliability)` |
| 온디바이스 추론 (연산) | ECG-FM 90.9M, CPU 637ms/window = 16x 실시간 (모바일 변환·전력은 미검증) |
| 검출 실증 | **AF만**(CACHET) — 낙상·저산소·2리드는 실세계 검증 연구 |
| 산출물 | `reliability_head.pt`, `p1_cardiac_channel.py`, `p1_cardiac_logging_adapter.py` |
...
├── lora_no_rlm/lora_best.pt                           epoch=16, AUROC=0.9436 (ablation)
├── gate/gate_best.pt                                  epoch=49, AUROC=0.8406 (게이트)
